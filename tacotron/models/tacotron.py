...
def split_func(x, split_pos, split_infos):
  rst = []
  start = 0
...
class Tacotron():
  ...
  def initialize(self, inputs, input_lengths, mel_targets=None, stop_token_targets=None, linear_targets=None, targets_lengths=None, gta=False,
    ...
    with tf.device(split_device):
      ...
      # tower_input_lengths = tf.random_uniform(
      #   shape = (1, batch_size),
      #   minval = 250,
      #   maxval = 251,
      #   dtype = tf.int32
      ...
      # tower_targets_lengths = tf.random_uniform(
      #   shape = (1,batch_size),
      #   minval = 500,
      #   maxval = 501,
      #   dtype = tf.int32
     ...
     # log('inputs: {}'.format(inputs))
     # log('mel_targets: {}'.format(mel_targets))
     # log('stop_token_targets: {}'.format(stop_token_targets))
     # log('linear_targets: {}'.format(linear_targets))

      #p_inputs = tf.py_func(split_func, [inputs, split_infos[:, 0], split_infos], lout_int)
     # p_mel_targets = tf.py_func(split_func, [mel_targets, split_infos[:,1], split_infos], lout_float) if mel_targets is not None else mel_targets
     # p_stop_token_targets = tf.py_func(split_func, [stop_token_targets, split_infos[:,2], split_infos], lout_float) if stop_token_targets is not None else stop_token_targets
     # p_linear_targets = tf.py_func(split_func, [linear_targets, split_infos[:,3], split_infos], lout_float) if linear_targets is not None else linear_targets
     ...
     # log('p_linear_targets: {}'.format(p_linear_targets))
      ...
      #c_inputs = []
      #c_mel_targets = []
      #c_stop_token_targets = []
      #c_linear_targets = []
      #for i in range(hp.tacotron_num_gpus):
       #  c_inputs.append(tf.reshape(p_inputs[i], [-1]))
       #  c_mel_targets.append(tf.reshape(p_mel_targets[i],[-1]))
       #  c_stop_token_targets.append(tf.reshape(p_stop_token_targets[i],[-1]))
       #  c_linear_targets.append(tf.reshape(p_linear_targets[i],[-1]))
      #log('c_inputs: {}'.format(c_inputs))
      #log('c_mel_targets: {}'.format(c_mel_targets))
      #log('c_stop_token_targets: {}'.format(c_stop_token_targets))
      #log('c_linear_targets: {}'.format(c_linear_targets))

      
      # convert to tensorflow own op impl
      # p_inputs = []
      # y_start = 0
      # x_pos = inputs.shape[0]
      # for i in range(hp.tacotron_num_gpus):
      #   y_pos = split_infos[i][0]
      #   p_inputs.append(tf.slice(inputs, [0, y_start], [x_pos, y_pos]))
      #   y_start = y_pos
      ...
      #   p_mel_targets = []
      #   y_start = 0
      #   x_pos = mel_targets.shape[0]
      #   for i in range(hp.tacotron_num_gpus):
      #     y_pos = split_infos[i][1]
      #     p_mel_targets.append(tf.slice(mel_targets, [0, y_start], [x_pos, y_pos]))
      #     y_start = y_pos

      # p_stop_token_targets = None
      # if stop_token_targets is not None:
      #   p_stop_token_targets = []
      #   y_start = 0
      #   x_pos = stop_token_targets.shape[0]
      #   for i in range(hp.tacotron_num_gpus):
      #     y_pos = split_infos[i][2]
      #     p_stop_token_targets.append(tf.slice(stop_token_targets, [0, y_start], [x_pos, y_pos]))
      #     y_start = y_pos

      # p_linear_targets = None
      # if linear_targets is not None:
      #   p_linear_targets = []
      #   y_start = 0
      #   x_pos = linear_targets.shape[0]
      #   for i in range(hp.tacotron_num_gpus):
      #     y_pos = split_infos[i][3]
      #     p_linear_targets.append(tf.slice(linear_targets, [0, y_start], [x_pos, y_pos]))
      #     y_start = y_pos+
